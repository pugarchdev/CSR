...
export default function DepartmentInterestsPage() {
  ...
  const fetchInterests = async () => {
    setLoading(true);
    try {
      const data = await apiFetch<any[]>("/company-interests/list");
      setInterests(data);
      setError(null);
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-4 border-slate-200">
        <div>
          <h1 className="text-xl font-bold font-heading text-blue-950">Company Interest Received</h1>
          <p className="text-xs text-slate-500">Track and review corporate interest submissions for your department's posted requirements</p>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center bg-white p-3 rounded-lg border shadow-sm">
        <div className="relative max-w-xs w-full">
          ...
          <input
            type="text"
            placeholder="Search company or requirement..."
        ...
        <GovCardBody className="p-0">
          {filteredInterests.length === 0 ? (
            <div className="p-12 text-center text-slate-500">
              <p className="font-medium text-slate-700">No company interests received yet.</p>
              <p className="text-xs mt-1">Once requirements are approved and listed in the marketplace, corporates can submit funding interest.</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-slate-200 text-sm">
                <thead className="bg-slate-50 font-bold text-slate-700">
                  <tr>
                    <th className="px-6 py-3 text-left">Company Sponsor</th>
                    <th className="px-6 py-3 text-left">Requirement Title</th>
                    <th className="px-6 py-3 text-left">Pledged Budget</th>
                  ...
                  {filteredInterests.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right">
                        <Button
                          onClick={() => router.push(`/department/requirements/${item.csrRequirementId}`)}
