...
import { Search, Eye, Landmark, Coins } from "lucide-react";
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-4 border-slate-200">
        <div>
          <h1 className="text-xl font-bold font-heading text-blue-950">Active Department CSR Projects</h1>
          <p className="text-xs text-slate-500">Track approved CSR projects converted from requirements, funding releases, and execution status</p>
        </div>
      </div>
              ...
              <table className="min-w-full divide-y divide-slate-200 text-sm">
                <thead className="bg-slate-50 font-bold text-slate-700">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left">NGO Partner</th>
                    <th className="px-6 py-3 text-left">Approved Budget</th>
                    ...
                    <tr key={proj.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-slate-600">
                        <span className="flex items-center gap-1">
                          ...
                          {proj.ngo?.name || "Pending Selection"}
                        </span>
                      </td>
                      <td className="px-6 py-4 font-semibold text-slate-800">
                        ₹{Number(proj.approvedBudget || 0).toLocaleString()}
                      </td>
