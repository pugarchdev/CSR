...
/* ── Motion Variants for Section Reveal & Text Sequence Reveal ── */
...
export default function LandingPageClient() {
  ...
  return (
    <div className="bg-slate-50 text-slate-700 min-h-screen font-sans relative overflow-hidden">
      ...
      <main className="mx-auto flex w-full max-w-[1380px] flex-col gap-10 sm:gap-14 px-3 sm:px-6 md:px-8 pb-16 pt-6 sm:pt-8 lg:pt-14 overflow-x-hidden">
        ...
        <Parallax3DSection>
          ...
          >
            <motion.div variants={textRevealSequence} className="flex flex-col gap-3 border-b border-slate-200/80 pb-5 md:flex-row md:items-end md:justify-between">
              <div>
                <div className="inline-flex items-center gap-1.5 text-[10px] font-extrabold uppercase tracking-widest text-blue-600 bg-blue-50 border border-blue-200/60 px-3 py-1 rounded-full mb-2">
                  <Sparkles size={12} className="text-blue-500" /> State-Led, District-Executed Convergence
                </div>
                <h2 className="break-words text-xl sm:text-3xl font-black leading-tight text-slate-900 tracking-tight">
            ...
            <motion.div
              ...
              whileInView="visible"
              viewport={{ once: true }}
              className="grid gap-3.5 sm:gap-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4"
            >
              {workflow.map((step, index) => (
                <motion.div key={step.title} variants={cardSlideFromRight}>
                  <div className="liquid-glass-card-light p-4 sm:p-6 relative h-full flex flex-col justify-between group rounded-2xl border border-slate-200/90 bg-white/80 backdrop-blur-md shadow-xs hover:shadow-xl transition-all duration-300">
                    <div>
                      <div className="flex items-center justify-between mb-3.5 sm:mb-4">
                        <div className={`p-2.5 sm:p-3 rounded-xl sm:rounded-2xl inline-flex items-center justify-center border shadow-sm ${step.glow} backdrop-blur-md transition-transform duration-300 group-hover:scale-110`}>
                          <step.icon size={20} className="sm:w-[22px] sm:h-[22px]" />
                        </div>
                        <span className="inline-flex items-center justify-center px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 text-[10px] sm:text-[11px] font-black text-white shadow-md shadow-blue-500/25 border border-white/40">
                          Step 0{index + 1}
                        </span>
                      </div>
                      <h3 className="text-sm sm:text-base font-extrabold text-slate-900 tracking-tight">{step.title}</h3>
                      <p className="mt-1.5 sm:mt-2 text-xs leading-relaxed text-slate-600 font-medium">{step.detail}</p>
                    </div>
                  </div>
                </motion.div>
              ))}
          ...
          >
            <div className="grid gap-5 sm:gap-8 md:grid-cols-2">
              ...
              <motion.div
                ...
                className="liquid-glass-card-light p-5 sm:p-8 relative overflow-hidden group border-blue-200/50 bg-gradient-to-br from-white/95 via-slate-50/95 to-blue-50/30 rounded-2xl sm:rounded-3xl border shadow-md hover:shadow-2xl transition-all"
              >
                {/* Photo graphic overlay with smooth gradient fade */}
                <div 
                  className="absolute inset-0 bg-cover bg-right opacity-10 group-hover:opacity-[0.14] transition-opacity duration-500 pointer-events-none rounded-2xl sm:rounded-3xl"
                ...
                <div className="relative z-10 flex flex-col justify-between h-full min-h-[240px] sm:min-h-[280px]">
                  <div className="flex flex-col gap-3.5 sm:gap-5">
                    <div className="grid h-12 w-12 sm:h-14 sm:w-14 place-items-center rounded-xl sm:rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-500/25 transition-transform duration-300 group-hover:scale-110 border border-white/40">
                    ...
                    <div>
                      ...
                      <h3 className="mt-2.5 text-xl sm:text-2xl font-black text-slate-900 tracking-tight">Partner with Maharashtra</h3>
                      <p className="mt-2 text-xs sm:text-sm leading-relaxed text-slate-600 max-w-lg font-medium">
                  ...
                  <div className="mt-6 sm:mt-8">
                    <Link
                      ...
                      className="inline-flex min-h-11 sm:min-h-12 items-center justify-center rounded-full bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 hover:from-blue-700 hover:to-indigo-700 px-6 sm:px-7 text-xs font-black uppercase tracking-wider text-white shadow-lg shadow-blue-500/20 hover:no-underline transition-all hover:scale-105 border border-white/30 w-full sm:w-auto text-center"
                    >
                      Submit Partnership Enquiry <ArrowRight size={15} className="ml-2" />
                    </Link>
                  </div>
                </div>
              </motion.div>
              ...
              <motion.div
                ...
                whileHover={{ y: -4, scale: 1.01 }}
                transition={{ type: "spring", stiffness: 200, damping: 18 }}
                className="liquid-glass-card-light p-5 sm:p-8 relative overflow-hidden group border-amber-200/50 bg-gradient-to-br from-white/95 via-slate-50/95 to-amber-50/30 rounded-2xl sm:rounded-3xl border shadow-md hover:shadow-2xl transition-all"
              >
                {/* Photo graphic overlay with smooth gradient fade */}
                <div 
                  className="absolute inset-0 bg-cover bg-right opacity-10 group-hover:opacity-[0.14] transition-opacity duration-500 pointer-events-none rounded-2xl sm:rounded-3xl"
                ...
                <div className="relative z-10 flex flex-col justify-between h-full min-h-[240px] sm:min-h-[280px]">
                  <div className="flex flex-col gap-3.5 sm:gap-5">
                    <div className="grid h-12 w-12 sm:h-14 sm:w-14 place-items-center rounded-xl sm:rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 text-white shadow-lg shadow-amber-500/25 transition-transform duration-300 group-hover:scale-110 border border-white/40">
                      <Landmark size={24} className="sm:w-[26px] sm:h-[26px]" />
                    </div>
                    <div>
                      <span className="text-[10px] font-extrabold uppercase tracking-widest text-amber-700 bg-amber-50 px-2.5 py-0.5 rounded-full border border-amber-200/60">
                        For Government Departments
                      </span>
                      <h3 className="mt-2.5 text-xl sm:text-2xl font-black text-slate-900 tracking-tight">Pitch a Development Need</h3>
                      <p className="mt-2 text-xs sm:text-sm leading-relaxed text-slate-600 max-w-lg font-medium">
            ...
            <motion.div variants={cardSlideFromRight} className="mt-5 p-4 sm:p-6 rounded-2xl sm:rounded-3xl liquid-glass-card-light flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3.5 sm:gap-4 border border-slate-200/80 bg-white/80 backdrop-blur-md shadow-sm">
              <div className="flex items-center gap-3">
                <div className="p-2.5 sm:p-3 bg-emerald-50 text-emerald-600 rounded-xl sm:rounded-2xl border border-emerald-200/60 shadow-sm shrink-0">
                  <ClipboardCheck size={20} className="sm:w-[22px] sm:h-[22px]" />
                </div>
                <div>
                  <h4 className="text-xs sm:text-sm font-extrabold text-slate-900 tracking-tight">Track an existing enquiry or pitch?</h4>
                  <p className="text-[11px] sm:text-xs text-slate-500 font-medium">Enter your tracking ID to view real-time convergence status.</p>
                </div>
              </div>
              <Link
                href="/track"
                className="liquid-glass-pill-btn inline-flex min-h-10 items-center justify-center px-6 text-xs font-bold text-slate-800 hover:text-blue-600 hover:no-underline w-full sm:w-auto rounded-full border border-slate-200 bg-slate-50 text-center"
              >
              ...
              {resources.map((item) => (
                <motion.div key={item.title} variants={cardSlideFromRight}>
                  <Link href={item.href} className="hover:no-underline flex h-full">
                    <div className="liquid-glass-card-light p-4 sm:p-5 rounded-2xl border border-slate-200/90 bg-white/80 backdrop-blur-md flex items-start gap-3.5 w-full group transition-all hover:shadow-lg hover:border-blue-300">
                      <span className="h-2.5 w-2.5 sm:h-3 sm:w-3 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 shrink-0 mt-1 shadow-sm shadow-blue-500/40 border border-white/60" />
                      <div className="flex-1 min-w-0">
                        <h3 className="text-xs sm:text-sm font-extrabold text-slate-900 leading-snug group-hover:text-blue-600 transition-colors tracking-tight flex items-center justify-between">
                          {item.title}
                          <ExternalLink size={12} className="text-slate-400 group-hover:text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity shrink-0 ml-1" />
                        </h3>
                        <p className="mt-1 sm:mt-2 text-xs leading-relaxed text-slate-600 font-medium">{item.description}</p>
                      </div>
                    </div>
                  </Link>
                </motion.div>
        ...
        <div className="grid gap-5 sm:gap-6 xl:grid-cols-[0.95fr_1.05fr] w-full min-w-0">
          <Parallax3DSection className="w-full min-w-0">
            ...
            >
              <div className="liquid-glass-card-light p-5 sm:p-8 h-full rounded-2xl sm:rounded-3xl w-full min-w-0 overflow-hidden border border-slate-200/90 bg-white/80 backdrop-blur-md shadow-sm">
                <div className="flex flex-col gap-1.5">
                  ...
                  <h3 className="break-words text-lg sm:text-xl font-black text-slate-900 tracking-tight mt-1">
                ...
                <div className="mt-5 sm:mt-6 flex flex-col gap-2.5 sm:gap-3">
                  {recommendations.map((item) => (
                    <motion.div
                      ...
                      className="flex items-start gap-3 rounded-xl sm:rounded-2xl border border-slate-200/80 bg-white/70 backdrop-blur-md p-3 sm:p-4 transition-all hover:bg-white hover:border-blue-300 cursor-default shadow-sm"
                    >
                  ...
                  <table className="w-full min-w-[360px] sm:min-w-[500px] border-collapse text-left text-xs">
                    <thead>
                      <tr className="bg-slate-100/60 text-slate-900 border-b border-slate-200/80">
                        <th className="px-3 py-2.5 sm:px-3.5 sm:py-3 font-extrabold">Title</th>
                        <th className="px-2.5 py-2.5 sm:px-3.5 sm:py-3 font-extrabold">Category</th>
                        <th className="px-2.5 py-2.5 sm:px-3.5 sm:py-3 font-extrabold whitespace-nowrap">Date</th>
                      </tr>
                    </thead>
        ...
        <Parallax3DSection>
          ...
          >
            ...
            <motion.div
              ...
              whileInView="visible"
              viewport={{ once: true }}
              className="grid gap-3.5 sm:gap-5 grid-cols-1 md:grid-cols-3"
            >
              {pillars.map((pillar, index) => (
                <motion.div key={pillar.title} variants={cardSlideFromRight}>
                  <div className="liquid-glass-card-light p-4 sm:p-6 h-full flex flex-col justify-between group rounded-2xl border border-slate-200/90 bg-white/80 backdrop-blur-md shadow-xs hover:shadow-xl transition-all">
                    <div>
                      <div className="flex items-center justify-between mb-3.5 sm:mb-4">
                        <div className="grid h-10 w-10 sm:h-12 sm:w-12 shrink-0 place-items-center rounded-xl sm:rounded-2xl border border-blue-200/80 bg-blue-50 text-blue-600 shadow-sm backdrop-blur-md transition-transform duration-300 group-hover:scale-110">
                          <pillar.icon size={20} className="sm:w-[22px] sm:h-[22px]" />
                        </div>
                        <span className="inline-flex items-center justify-center px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full bg-blue-50 text-blue-600 border border-blue-200/60 text-[10px] sm:text-[11px] font-black tracking-wider">
                          Pillar 0{index + 1}
                        </span>
                      </div>
                      <h3 className="text-sm sm:text-base font-extrabold text-slate-900 tracking-tight">{pillar.title}</h3>
                      <p className="mt-1.5 sm:mt-2 text-xs font-medium leading-relaxed text-slate-600">{pillar.detail}</p>
                    </div>
                    <Link href="/knowledge" className="mt-4 sm:mt-5 inline-flex items-center gap-1.5 text-xs font-bold text-blue-600 hover:underline">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-950 p-5 sm:p-10 md:p-12 rounded-2xl sm:rounded-3xl relative overflow-hidden border border-white/10 shadow-2xl">
            ...
            <div className="relative z-10 flex flex-col gap-5 sm:gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex flex-col gap-3.5 sm:flex-row sm:items-center sm:gap-6">
                <div className="h-12 w-12 sm:h-14 sm:w-14 rounded-xl sm:rounded-2xl bg-gradient-to-br from-blue-500/20 to-indigo-500/20 backdrop-blur-xl flex items-center justify-center shrink-0 border border-white/20 shadow-lg">
                  <Headphones className="text-blue-300" size={24} />
                </div>
                <div>
                  <h2 className="text-base sm:text-xl lg:text-2xl font-black text-white tracking-tight leading-snug">
                    Need help partnering with Maharashtra or pitching a development need?
                  </h2>
                  <p className="mt-1.5 sm:mt-2 max-w-2xl text-xs sm:text-sm font-medium leading-relaxed text-slate-300">
              ...
              <div className="flex flex-col sm:flex-row gap-2.5 sm:gap-3 shrink-0 w-full sm:w-auto">
                <Link href="/knowledge" className="inline-flex min-h-11 items-center justify-center rounded-full border border-white/20 px-6 text-xs font-bold text-white hover:bg-white/10 hover:no-underline transition-all backdrop-blur-md w-full sm:w-auto text-center">
                  Knowledge Center
                </Link>
                <Link href="/contact" className="inline-flex min-h-11 items-center justify-center rounded-full bg-gradient-to-r from-amber-500 to-orange-500 px-6 text-xs font-black uppercase tracking-wider text-white hover:from-amber-600 hover:to-orange-600 hover:no-underline shadow-lg shadow-orange-500/25 transition-all hover:scale-105 border border-white/20 w-full sm:w-auto text-center">
                  Contact Helpdesk
                </Link>
              </div>
            </div>
