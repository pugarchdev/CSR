...
export async function sendUserInvitationEmail(payload: UserInvitationEmailInput): Promise<{ messageId: string; response: string }> {
        ...
        <div class="container">
          ...
          <div class="body">
            ...
            <div class="detail-box">
              ...
              ${payload.loginIdentifier && payload.loginIdentifier !== payload.to ? `
              <div class="detail-row">
                <span class="detail-label">Login Identifier:</span>
                <span><code style="font-family: monospace; background: #e2e8f0; padding: 2px 6px; border-radius: 4px;">${payload.loginIdentifier}</code></span>
              </div>` : ""}
              <div class="detail-row">
            ...
            <p style="font-size: 13px; color: #475569;">
              ${payload.isAutogenerated 
                ? "A temporary password has been autogenerated for your account. You can log in directly using your email (or Login Identifier) and the password above, or reset your password using the activation link." 
  ...
  // Log autogenerated / account credentials to terminal console
  console.log(`\n======================================================`);
  console.log(`🔑 [ACCOUNT CREDENTIALS / INVITATION EMAIL DISPATCH]`);
  console.log(`📧 Recipient Email: ${payload.to}`);
  if (payload.loginIdentifier) {
    console.log(`🆔 Login Identifier: ${payload.loginIdentifier}`);
  }
  console.log(`👤 Name: ${payload.applicantName}`);
  console.log(`🛡️ Assigned Role: ${payload.roleName}`);
  if (payload.password) {
    console.log(`🔒 ${payload.isAutogenerated ? "Autogenerated Password" : "Password"}: ${payload.password}`);
  }
  if (payload.resetUrl) {
    console.log(`🔗 Activation / Reset URL: ${payload.resetUrl}`);
  }
  console.log(`======================================================\n`);

  let info: { messageId: string; response: string } = { messageId: "dev-mock-id", response: "OK" };
  try {
    info = await transporter.sendMail(mailOptions);
  } catch (err: any) {
    console.warn(`[EMAIL DELIVERY WARN] Failed to send email to ${payload.to} (${err?.message}), but credentials were logged to terminal.`);
  }
...
export interface NodalOfficerAssignmentPayload {
  to: string;
  officerName: string;
  password?: string;
  isAutogenerated?: boolean;
...
export async function sendNodalOfficerAssignmentEmail(payload: NodalOfficerAssignmentPayload): Promise<{ messageId: string; response: string }> {
  const portalLogo = "https://mahacsr.maharashtra.gov.in/assets/logo.png";
  const supportEmail = "support.csr@maharashtra.gov.in";
  const subject = `[MahaCSR] Project Assignment & Portal Credentials — ${payload.projectCode}`;

  const loginUrl = getAbsoluteUrl(payload.loginUrl || "/login");
  const projectUrl = getAbsoluteUrl(payload.projectUrl || `/projects`);
  const authorityText = [payload.authorityDesignation, payload.departmentName].filter(Boolean).join(", ") || "the Competent Authority";
    ...
    <html>
      <head>
        ...
        <style>
          body { font-family: 'Inter', Helvetica, Arial, sans-serif; background-color: #f4f6f8; margin: 0; padding: 20px; color: #334e68; }
          .container { max-width: 600px; background: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.05); margin: 0 auto; }
          .header { background: #0d1c3a; padding: 28px; text-align: center; border-bottom: 4px solid #ff9800; }
          .logo { max-height: 55px; }
          .body { padding: 30px; line-height: 1.6; }
          .h1 { font-size: 20px; color: #102a43; font-weight: 700; margin-bottom: 12px; }
          .project-badge { background: #e0f2fe; color: #0369a1; padding: 6px 12px; border-radius: 6px; font-weight: bold; font-size: 13px; display: inline-block; margin-bottom: 15px; }
          .detail-box { background: #f8fafc; border: 1px solid #e2e8f0; border-left: 4px solid #0284c7; padding: 16px 20px; border-radius: 6px; margin: 20px 0; }
          .detail-row { margin-bottom: 8px; font-size: 14px; }
          .detail-label { font-weight: bold; color: #475569; display: inline-block; width: 140px; }
          .button-container { text-align: center; margin: 25px 0; }
          .btn { background: #0f172a; color: #ffffff !important; text-decoration: none; padding: 13px 28px; border-radius: 8px; font-weight: bold; font-size: 13px; display: inline-block; margin: 5px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
          .footer { background: #f8fafc; text-align: center; padding: 20px; font-size: 12px; color: #64748b; border-top: 1px solid #e2e8f0; }
          .footer a { color: #0284c7; text-decoration: none; font-weight: bold; }
        </style>
      </head>
      <body>
        <div class="container">
          ...
          <div class="body">
            <div class="project-badge">PROJECT ASSIGNMENT NOTICE</div>
            <div class="h1">CSR Project Nodal Officer Allocation</div>
            <p>Dear <strong>${payload.officerName}</strong>,</p>
            <p>You have been assigned as the <strong>District Nodal Officer</strong> for the following CSR Convergence Project by <strong>${authorityText}</strong>:</p>
            
            <div class="detail-box">
              <div class="detail-row">
                <span class="detail-label">Project Code:</span>
                <span><strong>${payload.projectCode}</strong></span>
              </div>
              <div class="detail-row">
                <span class="detail-label">Project Title:</span>
                <span><strong>${payload.projectTitle}</strong></span>
              </div>
              <div class="detail-row">
                <span class="detail-label">Assigned By:</span>
                <span>${authorityText}</span>
              </div>
              <div class="detail-row" style="margin-top: 12px; border-top: 1px dashed #cbd5e1; padding-top: 10px;">
                <span class="detail-label">Official Email:</span>
                <span><strong>${payload.to}</strong></span>
              </div>
              ${payload.password ? `
              <div class="detail-row">
                <span class="detail-label">Password:</span>
                <span style="font-family: monospace; background: #e2e8f0; padding: 3px 8px; border-radius: 4px; font-size: 15px; color: #0f172a;"><strong>${payload.password}</strong></span>
              </div>` : ""}
            </div>

            <p style="font-size: 13px; color: #475569;">
              ${payload.password ? "A temporary password has been generated for your login. Upon first login, you will be prompted to set a new password." : "You can log in to the portal using your existing credentials to view and manage this project."}
            </p>

            <div class="button-container">
              <a href="${loginUrl}" class="btn" target="_blank">Log In to View Assigned Project</a>
            </div>
          ...
          <div class="footer">
            <p>© 2026 Government of Maharashtra | Planning Department</p>
            <p>Need support? Contact <a href="mailto:${supportEmail}">${supportEmail}</a></p>
          </div>
        </div>
      </body>
    </html>
  ...
  console.log(`\n======================================================`);
  console.log(`📋 [NODAL OFFICER PROJECT ASSIGNMENT EMAIL]`);
  console.log(`📧 To: ${payload.to}`);
  console.log(`👤 Officer: ${payload.officerName}`);
  console.log(`📁 Project: ${payload.projectTitle} (${payload.projectCode})`);
  console.log(`🏛️ Assigned By: ${authorityText}`);
  if (payload.password) {
    console.log(`🔒 Temp Password: ${payload.password}`);
  }
  console.log(`======================================================\n`);
  ...
  try {
    ...
    console.warn(`[EMAIL DELIVERY WARN] Failed to send email to ${payload.to} (${err?.message}), but notification was logged.`);
  }
